...
std::string ControlFrame(const logic_opt::World& world, size_t t) {
  ...
  throw std::runtime_error(
      "WorkspaceConstraint::ControlFrame(): No control frame found.");
}
...
std::string TargetFrame(const logic_opt::World& world, size_t t) {
  ...
  throw std::runtime_error(
      "WorkspaceConstraint::TargetFrame(): No target frame found.");
}
...
WorkspaceConstraint::WorkspaceConstraint(World& world, size_t t_workspace,
                                         const std::string& name_ee)
    : FrameConstraint(
          kNumConstraints, kDof * (t_workspace + 1), t_workspace, kNumTimesteps,
...
double WorkspaceConstraint::ComputeError(
    Eigen::Ref<const Eigen::MatrixXd> X) const {
  const Eigen::Vector3d x_t =
      world_.Position(name_ee_, world_.kWorldFrame, X, t_start());
