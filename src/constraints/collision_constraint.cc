...
std::string ControlFrame(const logic_opt::World& world, size_t t) {
  ...
  throw std::runtime_error(
      "CollisionConstraint::ControlFrame(): No control frame found.");
}
...
std::string TargetFrame(const logic_opt::World& world, size_t t) {
  ...
  throw std::runtime_error(
      "CollisionConstraint::TargetFrame(): No target frame found.");
}
...
CollisionConstraint::CollisionConstraint(
    World& world, size_t t_collision, bool ignore_control_target,
    const std::set<std::string>& ignore_obstacles)
    : FrameConstraint(
          kNumConstraints, kLenJacobian, t_collision, kNumTimesteps,
  ...
  const ctrl_utils::Tree<std::string, Frame>& frames =
      world.frames(t_collision);
  for (const std::pair<std::string, Frame>& key_val : frames.values()) {
    const std::string& frame = key_val.first;
    if (frame == World::kWorldFrame ||
        !world_.objects()->at(frame).collision ||
        ignore_obstacles.find(frame) != ignore_obstacles.end())
      continue;
...
void CollisionConstraint::Jacobian(Eigen::Ref<const Eigen::MatrixXd> X,
  ...
  for (size_t i = 0; i < kDof; i++) {
    ...
    const double x_err_hp =
        ComputeDistance(X_h, ee_closest_, object_closest_, kMaxDist);
    x_it = x_it_0 - h;
    const double x_err_hn =
        ComputeDistance(X_h, ee_closest_, object_closest_, kMaxDist);
    x_it = x_it_0;
    const double dx_h =
        0.5 * (std::abs(x_err_hp) * x_err_hp - std::abs(x_err_hn) * x_err_hn) /
        (2. * h);
...
std::optional<ncollide3d::query::Contact> CollisionConstraint::ComputeError(
    Eigen::Ref<const Eigen::MatrixXd> X, std::string* out_ee_closest,
  ...
  for (const std::string& ee_frame : ee_frames_) {
    for (const std::string& object_frame : object_frames_) {
      if (ignore_control_target_ && ee_frame == control_frame() &&
      ...
      const double dist =
          ComputeDistance(X, ee_frame, object_frame, proximity_dist, &contact);
...
double CollisionConstraint::ComputeDistance(
  ...
  const Object& ee = world_.objects()->at(ee_frame);
  ...
  const auto contact = ncollide3d::query::contact(
      Eigen::Isometry3d::Identity(), *ee.collision, T_object_to_ee.eigen(),
