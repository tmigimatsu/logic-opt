...
TouchConstraint::TouchConstraint(World& world, size_t t_touch,
    ...
    : FrameConstraint(kNumConstraints, kLenJacobian, t_touch, kNumTimesteps,
                      name_control, name_target,
                      "constraint_t" + std::to_string(t_touch) + "_touch"),
      world_(world) {
  if (name_control == world.kWorldFrame) {
    throw std::invalid_argument(
        "TouchConstraint::TouchConstraint(): " + world.kWorldFrame +
...
void TouchConstraint::Evaluate(Eigen::Ref<const Eigen::MatrixXd> X,
  ...
  constraints(0) = -0.5 * std::abs(contact_->depth) *
                   contact_->depth;  // signed_dist < epsilon
...
void TouchConstraint::Jacobian(Eigen::Ref<const Eigen::MatrixXd> X,
  ...
  if (contact_->depth == 0.)
    throw std::runtime_error(name + "::Jacobian(): 0 depth.");
  ...
  for (size_t i = 3; i < kDof; i++) {
    ...
    if (contact_hp->depth == 0.)
      throw std::runtime_error(name + "::Jacobian(): 0 depth hp.");
    x_it = x_it_0 - kH;
    const auto contact_hn = ComputeError(X_h);
    const double x_err_hn = contact_hn ? -contact_hn->depth : 0.;
    if (contact_hn->depth == 0.)
      throw std::runtime_error(name + "::Jacobian(): 0 depth hn.");
    x_it = x_it_0;
    ...
    if (contact_->depth < 0 && std::abs(dx_h) > 0.5) {
      ...
      ss << "TouchConstraint::Jacobian(): Ill-conditioned J(" << i << ","
         << t_start() << "): " << dx_h << " " << contact_hn.has_value() << " "
         << x_err_hp << " " << x_err_hn << " " << x_err << std::endl;
...
std::optional<ncollide3d::query::Contact> TouchConstraint::ComputeError(
  ...
  return ncollide3d::query::contact(
      Eigen::Isometry3d::Identity(), *target.collision,
      T_control_to_target.eigen(), *control.collision, kMaxDist);
}
