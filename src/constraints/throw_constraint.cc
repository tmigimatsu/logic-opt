...
InitializeConstraints(logic_opt::World3& world, size_t t_throw,
                      const std::string& name_object, const std::string& name_target) {
  using namespace logic_opt;

  if (name_object == world.kWorldFrame) {
    throw std::invalid_argument("ThrowConstraint::ThrowConstraint(): " + world.kWorldFrame +
  ...
  constraints.emplace_back(new PlaceConstraint(world, t_throw, name_object, name_target));
...
ThrowConstraint::ThrowConstraint(World3& world, size_t t_place,
                                 const std::string& name_object, const std::string& name_target)
